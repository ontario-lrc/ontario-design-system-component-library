...
function parseCss(e, t) {
	...
	if (((e.parsedCssText = e.cssText = r.trim()), e.parent)) {
		...
		r = (r = (r = _expandUnicodeEscapes((r = t.substring(n, e.start - 1)))).replace(RX.multipleSpaces, ' ')).substring(
			r.lastIndexOf(';') + 1,
		);
			...
			e.atRule
				? 0 === s.indexOf(MEDIA_START)
					...
					: s.match(RX.keyframesRule) &&
					  ((e.type = types.KEYFRAMES_RULE), (e.keyframesName = e.selector.split(RX.multipleSpaces).pop()))
...
function normalizeValue(e) {
	...
	return t && (e = e.slice(0, e.length - IMPORTANT.length).trim()), { value: e, important: t };
}
	...
	return (
		...
		(e.prototype.createHostStyle = function (e, t, r, n) {
			...
			return (
				...
				s.usesCssVars
					? n
						? ((o['s-sc'] = t = ''.concat(s.scopeId, '-').concat(this.count)),
